...
def main():
	...
	args.name = f"eval_sgd_deit_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}"
		...
		for step, batch in enumerate(train_dataloader):
			...
			for (name, p) in model.named_parameters():
				if to_use(name):
					...
					mt[name] = m
					result_params[name] = p - grad.view(*p.size()) * args.scale
