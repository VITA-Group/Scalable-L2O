	...
	while True:
		...
		with torch.autograd.set_detect_anomaly(True):
			...
			for i in range(ro):
				...
				logits = model(_input)[0]
				_loss = torch.nn.functional.cross_entropy(logits, _label) / ro
				_loss.backward(retain_graph=True)
				accumu += _loss.item()
			
			wandb.log({"meta_train/train_loss": accumu}, step=train_step)
			train_step += 1
			avg_lm_loss.update(accumu)
		...
		if (current_idx + 1) % unroll == 0:
			...
			for name in result_params:
				if False:
					rsetattr(model, name, nn.Parameter(result_params[name]))
				else:
...
if __name__ == '__main__':
	...
	args.name = f"opt_vit_imagenet_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.work_dir = f"./trained_models/opt_vit_imagenet_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
