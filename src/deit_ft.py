...
def main():
	...
	args.name = f"opt_deit_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}"
	args.work_dir = f"./trained_models/opt_deit_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}"
	...
	for epoch in range(args.num_train_epochs):
		...
		while True:
			...
			accu_loss = 0
			roll = int(128 / args.per_device_train_batch_size)
			for i in range(roll):
				try:
				...
				accu_loss += loss.item() / roll
			wandb.log({"meta_train/train_loss": accu_loss}, step=completed_steps)
			result_params = {}            
			...
			for (name, p) in model.named_parameters():
				if to_use(name):
					...
					mt[name] = m
					mt_hat = m / (1-beta1**(current_idx))
					v.mul_(beta2).add_((1 - beta2) * (grad ** 2))
					vt[name] = v
					vt_hat = v / (1-beta2**(current_idx))
			...
			if (current_idx + 1) % args.unroll_length == 0:
				...
				log_str = '| epoch {:3d} step {:>8d} | {:>6d} batches ' \
									...
									epoch, completed_steps, current_idx + 1, 
									meta_loss) 
		...
		model_path = os.path.join(args.work_dir, 'model.'+str(completed_steps)+'.pt')
		...
		if eval_metric_value > best_metric_value:
			...
			torch.save({'optimizer_state_dict': opt_net.state_dict(), "epoch": epoch}, model_path)
