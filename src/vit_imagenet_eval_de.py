...
if __name__ == '__main__':
	...
	args.name = f"eval_opt_vit_imagenet_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.optimizer_checkpoint = f"./trained_models/opt_vit_imagenet_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}/best.pt"
	args.work_dir = f"./trained_models/eval_opt_vit_imagenet_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	...
	#assert False
	from original_vit.modeling import CONFIGS, VisionTransformer
	P = torch.load("vit_P.pth.tar").cuda()
	config = CONFIGS['ViT-B_16']
	model = VisionTransformer(config, 224, zero_head=True, num_classes=1000)
	state_dict= torch.load("imagenet_checkpoint_100.bin", map_location='cpu')
	new_state_dict = {}
	...
	def create_model():
		resnet = VisionTransformer(config, 224, zero_head=True, num_classes=1000)
		resnet.load_state_dict(torch.load("imagenet_checkpoint_100.bin", map_location='cpu'))
		return resnet
