...
if __name__ == '__main__':
	...
	args.name = f"opt_resnet18_cf100_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.work_dir = f"./trained_models/eval_opt_resnet18_cf100_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	...
	create_model = lambda: resnet18(num_classes=10)
	...
	#assert False
	P = torch.load("resnet18_CIFAR100_P.pth.tar")
	model = resnet18(num_classes=100)
	state_dict= torch.load("resnet18_CIFAR100_0.pt")
	new_state_dict = {}
