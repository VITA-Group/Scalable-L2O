...
def train_validate(model, opt_net, optimizer, scheduler, meta_optimizer, train_loader, valid_loader, args, create_model, init_weight, P, train_step = 0, epoch = 0, unroll = 5, beta1 = 0.9, beta2 = 0.99, best_val_accuracy=None):
	...
	while True:
		...
		with torch.autograd.set_detect_anomaly(True):
			...
			wandb.log({"meta_train/train_loss": accu_loss}, step=train_step)
...
def main():
	...
	parser.add_argument("--meta_train_eval_epoch", default=5, type=int)
	...
	args.work_dir = f"./trained_models/opt_vit_16_b_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	...
	for epoch in itertools.count(start=1):
		...
		train_step, best_val_accuracy = train_validate(model, opt_net, None, None, meta_optimizer, train_loader, test_loader, args, create_model, None, None, train_step=train_step, epoch = epoch, unroll=args.unroll_length, best_val_accuracy=best_val_accuracy)
