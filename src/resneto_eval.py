...
if __name__ == '__main__':
	...
	args.name = f"eval_opt_resnet18_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	...
	for name, p in model.named_parameters():
		if to_use(name):
			...
			hidden_states.append([torch.zeros(p.nelement(), opt_net.hidden_sz).to(args.local_rank)])
			cell_states.append([torch.zeros(p.nelement(), opt_net.hidden_sz).to(args.local_rank)])
