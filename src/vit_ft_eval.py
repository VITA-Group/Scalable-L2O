		...
		with torch.autograd.set_detect_anomaly(True):
			...
			for (name, p) in model.named_parameters():
				if to_use(name):
					...
					result_h[name] = nh
					result_c[name] = nc
					# TODO: recover back to normal after public
					# Current: switch to mom after 5000 steps
					if train_step > 5000:
						result_params[name] = p - m * 0.01 
					else:
...
def main():
	...
	args.optimizer_checkpoint = f"./trained_models/opt_vit_16_b_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}/best.pt"
