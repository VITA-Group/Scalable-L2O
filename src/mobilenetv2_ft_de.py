...
if __name__ == '__main__':
	...
	args.name = f"opt_mobilenetv2_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.work_dir = f"./trained_models/opt_mobilenetv2_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	...
	P = torch.load("mobilenetv2_CIFAR10_P.pth.tar")
	model = mobilenetv2(num_class=10)
	state_dict= torch.load("mobilenetv2_CIFAR10_0.pt")
	new_state_dict = {}
	...
	def create_model():
		resnet = mobilenetv2(num_class=10)
