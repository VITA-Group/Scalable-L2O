...
if __name__ == '__main__':
	...
	args.name = f"opt_eval_vgg16_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.work_dir = f"./trained_models/eval_opt_vgg16_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	args.optimizer_checkpoint = f"./trained_models/opt_vgg16_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}/best.pt"
	if args.use_second_layer:
	...
	#assert False
	P = torch.load("vgg16_CIFAR10_P.pth.tar")

	model = vgg16_bn(num_classes=10)
	state_dict= torch.load("vgg16_CIFAR10_0.pt")
	new_state_dict = {}
	for key in state_dict:
		if key.startswith("module."):
			new_state_dict[key[7:]] = state_dict[key]
	def create_model():
		resnet = vgg16_bn(num_classes=10)
