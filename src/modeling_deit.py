...
class DeiTEmbeddings(MetaModule):
    """
...
class PatchEmbeddings(MetaModule):
    """
    ...
    def __init__(self, image_size=224, patch_size=16, num_channels=3, embed_dim=768):
        ...
        self.projection = MetaConv2d(num_channels, embed_dim, kernel_size=patch_size, stride=patch_size)
...
class DeiTSelfAttention(MetaModule):
    def __init__(self, config):
        super().__init__()
...
class DeiTSelfOutput(MetaModule):
    """
...
class DeiTAttention(MetaModule):
    def __init__(self, config):
        super().__init__()
...
class DeiTIntermediate(MetaModule):
    def __init__(self, config):
        super().__init__()
...
class DeiTEncoder(MetaModule):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = MetaModuleList([DeiTLayer(config) for _ in range(config.num_hidden_layers)])
...
class DeiTPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (MetaLinear, nn.Conv2d)):
        ...
        elif isinstance(module, DeiTSelfAttention):
...
DEIT_START_DOCSTRING = r"""
    This model is a PyTorch `torch.MetaModule <https://pytorch.org/docs/stable/nn.html#torch.MetaModule>`_ subclass. Use
...
class DeiTForImageClassification(DeiTPreTrainedModel):
    def __init__(self, config):
        ...
        # Classifier head
        self.classifier = MetaLinear(config.hidden_size, config.num_labels) if config.num_labels > 0 else nn.Identity()
...
class DeiTForImageClassificationWithTeacher(DeiTPreTrainedModel):
    def __init__(self, config):
        ...
        # Classifier heads
        self.cls_classifier = (
            MetaLinear(config.hidden_size, config.num_labels) if config.num_labels > 0 else nn.Identity()
        )
