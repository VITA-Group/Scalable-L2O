...
def train_validate(model, opt_net, optimizer, scheduler, meta_optimizer, train_loader, valid_loader, args, create_model, init_weight, P, mt, vt, hidden_states, cell_states, train_step = 0, epoch = 0, unroll = 5, beta1 = 0.9, beta2 = 0.99, best_val_accuracy=None):
	...
	for idx, data in enumerate(train_loader):
		...
		with torch.autograd.set_detect_anomaly(True):
			...
			result_params = {}
			all_params_gradients = torch.autograd.grad(_loss, model.parameters(), retain_graph=False)
			
			flatten = []
			for p in all_params_gradients:
				flatten.append(p.reshape(-1,1))
			...
			mt_hat = mt[0] / (1-beta1**(idx + 1))
			vt[0].mul_(beta2).add_((1 - beta2) * (grad ** 2))
			vt_hat = vt[0] / (1-beta2**(idx + 1))
...
def calculate_basis(model: MetaModule, opt_net, optimizer, scheduler, meta_optimizer, train_loader, valid_loader, args, create_model, init_weight, train_step = 0, epoch = 0, unroll = 5, beta1 = 0.9, beta2 = 0.99, best_val_accuracy=None):
	model.train()
	opt_net.train()
	meta_optimizer.zero_grad()
	avg_lm_loss = AverageMeter()
...
if __name__ == '__main__':
	...
	args.name = f"opt_eval_resnet50_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.work_dir = f"./trained_models/eval_opt_resnet50_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	args.optimizer_checkpoint = f"./trained_models/opt_resnet50_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}/best.pt"
	if args.use_second_layer:
	...
	#assert False
	P = torch.load("resnet50_P.pth.tar")
	model = resnet50(num_classes=10)
	state_dict= torch.load("resnet50_0.pt")
	new_state_dict = {}
