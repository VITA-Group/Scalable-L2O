...
if __name__ == '__main__':
	...
	train_loader, test_loader = setup_model_dataset(args, dataset='cifar10')
	
	
	args.name = f"opt_vgg16_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.work_dir = f"./trained_models/opt_vgg16_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	...
	create_model = lambda: vgg16_bn(num_classes=10)
