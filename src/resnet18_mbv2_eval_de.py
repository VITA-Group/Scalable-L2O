...
if __name__ == '__main__':
	...
	args.name = f"opt_eval_resnet18_mbv2_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.work_dir = f"./trained_models/eval_opt_resnet18_mbv2_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	args.optimizer_checkpoint = f"./trained_models/opt_mobilenetv2_ul3_ts1000_hz_8_dim16_sc0.0001_al32_mlr0.01_bs128/best.pt"
	if args.use_second_layer:
