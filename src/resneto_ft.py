...
def train_validate(model, opt_net, optimizer, scheduler, meta_optimizer, train_loader, valid_loader, args, create_model, init_weight, train_step = 0, epoch = 0, unroll = 5, beta1 = 0.9, beta2 = 0.99, best_val_accuracy=None):
	...
	for name, p in model.named_parameters():
		if to_use(name):
			...
			hidden_states.append([torch.zeros(p.nelement(), opt_net.hidden_sz, requires_grad=True).to(args.local_rank)])
			cell_states.append([torch.zeros(p.nelement(), opt_net.hidden_sz, requires_grad=True).to(args.local_rank)])
...
if __name__ == '__main__':
	...
	args.name = f"opt_resnet18_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	args.work_dir = f"./trained_models/opt_resnet18_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	args.logging = create_exp_dir(args.work_dir)
	...
	model = resnet18(num_classes=10, lora_dim=args.lora_dim, pretrained=True)
	create_model = lambda: resnet18(num_classes=10, lora_dim=args.lora_dim, pretrained=True)
