...
class BasicBlock(MetaModule):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A', lora_dim=32):
        ...
        self.conv1_adapter_1 = MetaLinear(lora_dim, planes, bias=None)
        self.conv1_adapter_2 = MetaLinear(in_planes * 9, lora_dim, bias=None)

        self.bn1 = MetaBatchNorm2d(planes)
        self.conv2 = MetaConv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)

        self.conv2_adapter_1 = MetaLinear(lora_dim, planes, bias=None)
        self.conv2_adapter_2 = MetaLinear(planes * 9, lora_dim, bias=None)
    ...
    def forward(self, x):
        conv1_weight = self.conv1.weight + int(os.environ['lora_alpha']) * (self.conv1_adapter_1.weight @ self.conv1_adapter_2.weight).view(*self.conv1.weight.shape)
        
        conv2_weight = self.conv2.weight + int(os.environ['lora_alpha']) * (self.conv2_adapter_1.weight @ self.conv2_adapter_2.weight).view(*self.conv2.weight.shape)
        out = F.conv2d(x, conv1_weight, None, stride=self.stride, padding=1)
        out = F.relu(self.bn1(out))
        out = F.conv2d(out, conv2_weight, None, stride=1, padding=1)
        out = self.bn2(out)
        out += self.shortcut(x)
        out = F.relu(out)
        return out
