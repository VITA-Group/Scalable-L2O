...
def train_validate(model, opt_net, optimizer, scheduler, meta_optimizer, train_loader, valid_loader, args, create_model, init_weight, P, mt, vt, hidden_states, cell_states, train_step = 0, epoch = 0, unroll = 5, beta1 = 0.9, beta2 = 0.99, best_val_accuracy=None):
	...
	for idx, data in enumerate(train_loader):
		...
		with torch.autograd.set_detect_anomaly(True):
			...
			# Xs.append(torch.cat([mt_tilde, gt_tilde], 1))
			...
			# 	Ys = torch.cat(Ys, 0).detach().cpu().numpy()
...
if __name__ == '__main__':
	...
	args.name = f"opt_eval_resnet20_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	if args.use_second_layer:
		args.name = args.name + "_second"
	args.work_dir = f"./trained_models/eval_opt_resnet20_cf10_cf100_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}"
	args.optimizer_checkpoint = f"./trained_models/opt_resnet20_cf100_ul{args.unroll_length}_ts{args.training_steps}_hz_{args.hidden_sz}_dim{args.lora_dim}_sc{args.scale}_al{args.lora_alpha}_mlr{args.meta_lr}_bs{args.batch_size}/best.pt"
	if args.use_second_layer:
